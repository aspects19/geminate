...
pub fn init_db() -> Result<()> {
    let conn = Connection::open("geminate.db")?;
...
pub fn add_chat(conn: &Connection) -> Result<i64> {
    conn.execute("INSERT INTO chats DEFAULT VALUES", [])?;
    Ok(conn.last_insert_rowid()) 
}

pub fn add_message(conn: &Connection, chat_id: i64, role: &str, content: &str) -> Result<(), Error> {
    conn.execute(
...
pub fn get_chats(conn: &Connection) -> Result<Vec<(i64, String)>> {
    ...
    let chats = stmt
        .query_map([], |row| Ok((row.get(0)?, row.get(1)?)))? // Return tuple properly
        .collect::<Result<Vec<_>, _>>()?; // Collect into Vec
